...
public class WalletService implements IWalletService {
	...
	public Wallet 조회_ETH잔액동기화(final String 지갑주소) {
		Wallet wallet = walletRepository.searchWallet(지갑주소);
		if (wallet == null)
			throw new NotFoundException(지갑주소 + " 해당 주소 지갑을 찾을 수 없습니다.");
		/**
		 * TODO 이더리움으로부터 잔액을 조회하여 잔액정보가 다를 경우 정보를 갱신하여 반환한다. ethereumService.java에 추가
		 * 메소드를 구현하는 것을 권장한다.
		 */
		BigDecimal curWei = new BigDecimal(this.ethereumService.getBalance(지갑주소));
		if (wallet.get잔액() != curWei) {
			walletRepository.잔액갱신(지갑주소, curWei);
		}
		wallet.set잔액(curWei);
	...
	public Wallet 잔액갱신(final String 지갑주소, final BigDecimal 잔액) {
		...
		return this.walletRepository.searchWallet(지갑주소);
	}
