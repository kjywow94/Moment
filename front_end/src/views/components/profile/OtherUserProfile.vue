...
import ReviewCard from "@/views/components/profile/OtherUserReviewCard";
...
export default {
  ...
  mounted() {
    //사용자의 정보를 가져온다.
    UserProfileService.getUserProfile(this.$route.params.id)
      .then(userProfile => {
        this.userProfile = userProfile.data;
        this.facebook = this.userProfile.sns1;
        this.instagram = this.userProfile.sns2;
      })
      .catch(err => {
        console.log("userProfile error : ", err);
      });
    //사용자의 사진을 가져온다.
    UserProfileService.getUserImage(this.$route.params.userEmail)
      .then(userImg => {
